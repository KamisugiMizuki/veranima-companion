...
桌宠设置页（`pet/settings.html`）的八个页面：角色与桌宠、模型连接、语音、记忆与人格、在场与主动、日程与生活、图片与表情、高级。模型连接测试成功后从 API 返回的模型列表下拉选择；API key 读取时只显示掩码；路径类配置使用原生文件/目录浏览器。

安卓端设置（`Settings.kt`，2026-09-20 核对）：主页面为语言模型 / Embedding / 搜索（博查，安卓唯一后端）、当前活跃角色、共享记忆备份（导出·导入全角色共同记忆）、共同记忆库（向量总览·密度·时间轴）、用户画像（13 键 + 锁定）、记忆待复核队列、电池优化白名单、使用情况访问（前台感知授权）；模型与搜索各有独立详情页带「测试」按钮。
