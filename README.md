...
- LLM/连接故障的可见兜底只属于当前轮，不写入 assistant 历史、长期记忆、风格学习或后续 prompt；模型复述旧故障文案时会被统一替换，旧数据库记录也在 `recent_messages()` 读取边界过滤。
- QQ 主动消息必须直接点明可靠素材中的具体主题；内部关系张力事件不能作为聊天素材，无法建立主题锚点时不发送。可在 `proactive.meal_reminders` 配置早餐约 08:00、午餐约 12:00、晚餐约 17:00，并在各锚点前后 10 分钟内稳定随机提醒；提醒仍受 QQ 静默时段、主动 Gate、每日上限和发送后去重约束。
...
- HTTP 200 但模型偶尔返回普通文本或错误 JSON 时不重复请求：交给对应通道解析器提取可见内容；混合“分析文本 + 多个 JSON 候选”时取最后一个有可见 `segments` 的对象；分析/内部指令只有在能提取明确最终答案时才放行，否则 fail-closed 且不写 assistant 历史；busy 场景只向模型注入“回复尽量简短”的偏好，完整性优先，不截断用户可见文本、不追加内部状态文案，并同步 `Reply` 对象；只有真正空响应、网络、鉴权或服务端错误才显示卡顿兜底。
