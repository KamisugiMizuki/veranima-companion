...
- 备份的是原文与状态。DB 快照虽连带向量，但向量只在**模型+维度一致**时才可信复用；
  否则导入端丢弃并按当前 embedding 模型全量重铸（memory_embedding 是派生数据）。
...
from ..memory.schema import unit_blob
...
def import_backup(root: Path, zip_path: Path, *, embedding_spec: str, embedding_dim: int,
    ...
    # （方案 B：memory_embedding 是普通表，无扩展依赖，双端同一条路径）
    con = sqlite3.connect(db_file)
    mem_n = con.execute("SELECT count(*) FROM memories WHERE content != ''").fetchone()[0]
    try:
        vec_n, min_len = con.execute(
            "SELECT count(*), min(length(embedding)) FROM memory_embedding").fetchone()
        old_dim = (min_len or 0) // 4
    except sqlite3.Error:
        vec_n, old_dim = 0, 0
    ...
    if need:
        ...
        con.execute("CREATE TABLE IF NOT EXISTS memory_embedding ("
                    "memory_id INTEGER PRIMARY KEY, embedding BLOB NOT NULL)")
        con.execute("DELETE FROM memory_embedding")
