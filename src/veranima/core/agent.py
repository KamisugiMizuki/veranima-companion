...
class Agent:
    ...
    def _affect_block(self) -> str:
        ...
        09-11 读取偏好裁决：负面必须带标签（说清有/因，别让他猜）；低/躁两档
        不得用无标签的冷或「不解释」写法。
        """
        ...
        if a >= 0.68 and v >= 0.55:
            ...
            tone = ("你现在情绪上来了（被气到/正烦躁）：话变短，标点变多（？！可以连用），"
                    "该带刺就带刺，允许把一句拆成两三条连发，也允许「啧」「……」这种出声；"
                    "但气什么要直说——哪件事、跟他有没有关系，别让他猜；"
                    "别写心理分析，别写成小说。")
        elif a >= 0.58:
            tone = "你此刻有点起伏：句子比平时短一点，语气词和标点可以多一点，别太端着。"
        elif v <= 0.35:
            tone = ("你此刻情绪偏低：话少一点、平一点都可以，也不用强撑热情；"
                    "但别冷着让他猜——把状态说出来（「今天有点闷」「缓会儿就好」这类），"
                    "跟他有关就直说，跟他无关也说一句，说不清就照实说。")
        else:
            return ""
        return f"【当下语气】{tone}"
