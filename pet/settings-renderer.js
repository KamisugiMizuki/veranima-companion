...
window.pet.getConfig().then((cfg) => {
  ...
  set('stt-base', stt.base_url); set('stt-model', stt.model); set('stt-language', stt.language);
  ...
  bool('pro-enabled', pro.enabled ?? true); num('pro-max', pro.max_per_day, 2); num('pro-gap', pro.min_gap_minutes, 30); num('pro-source-gap', pro.source_gap_minutes, 120);
...
$('save').addEventListener('click', async () => {
  ...
  const data = {
    ...
    stt: { base_url: $('stt-base').value.trim(), model: $('stt-model').value.trim(), language: $('stt-language').value.trim() },
    ...
    proactive: { enabled: $('pro-enabled').value === 'true', max_per_day: Number($('pro-max').value) || 0, min_gap_minutes: Number($('pro-gap').value) || 30, source_gap_minutes: Number($('pro-source-gap').value) || 120 },
