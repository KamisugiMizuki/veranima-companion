...
### 3.2 卡内容写作自检（2026-09-07 外部借鉴评估裁决并入）

角色卡 = 写给 LLM 读的文档。以下四条在填卡与审卡时逐条对照，只约束**写法**，
不约束写什么——规范的目的是让卡上声明的行为真的发生，不是让卡长得一样。

1. **指针措辞决定触发可靠性**：卡里「什么时候该做某行为」的条件句，比行为条文
   本身更多决定它是否真的发生。模糊指针（「适当关心」「根据情况」）= 方差 bug；
   条件要写成模型能对齐的具体信号。
2. **闭集 + 显式穷举**：任何枚举字段（状态值、画像键、词表标签）必须写全合法
   值并声明集外丢弃——运行时闸（如 judges `_VALID_*`、portrait 词表回退）已有，
   卡侧措辞与之同步，防「静默拒收」（画像 13→11 键的学费）。
3. **锚点词（leading words）**：给角色反复出现的行为/意象取短词（「护短」
   「屋顶时间」），全文用它而非每次展开描述——省 token 且比长描述更稳。
   锚点词是角色私有资产：**一条描述若原样搬到另一张卡里不违和，它就是通用
   条款，删掉或换成只有这个人说得出的写法**（防模板化的移植测试）。
4. **删空转条款**：不影响任何可观察输出的句子全部删（「她性格温柔」类），
   换成可观察行为条款；但可观察≠脚本化——禁止「被夸时先傲娇反驳再道谢」式
   if-then 行为配方，情绪差异交给运行时（state/关系阶段/学习参数），卡上只
   给倾向与边界。

禁止事项：本规范与任何文档里**不得放示例条款库**（模板被抄不是因为它存在，
是因为它好抄）。审卡以本文四条 + 各卡 `creators_note` 为准。
