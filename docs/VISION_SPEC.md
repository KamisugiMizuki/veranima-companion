...
> 实施：V1（d72e0be attention 包骨架/显著度/状态机/事件）、V2（34191b8 配置接入）、V3（vision.py 删除 + 测试迁移）
> 状态：**V1-V3 已完成（2026-08-19）**。attention 包已落地并替换 `vision.py`（已删除）。
...
#### 2.5.1 注意力焦点估计（2026-08-19 用户提出，已验证可行）

**鼠标位置 + 前台窗口**（Windows API，零依赖）是比像素变化更接近「注意力焦点」的信号：

- `GetCursorPos` → 鼠标绝对坐标（用户正在操作的位置，注意力大概率在此）
- `GetForegroundWindow` + `GetWindowTextW` → 前台窗口标题（任务上下文）

实现：presence.py 增加 `cursor_pos() -> (x, y)`（ctypes，已实测可用）。

焦点合成规则：
```
焦点 = 鼠标位置（若鼠标在某区域停留 > 2s）    # 操作焦点
     | 前台窗口标题栏区域（鼠标长期不动 > 30s） # 阅读/观看焦点
     | 显著度地图最高区域（两者都无信号时）     # 兜底
```

限制（诚实声明）：
- 鼠标 ≠ 注意力（用户可能看副屏/视频，鼠标停在原地）——但作为估计，比全屏像素差强得多
- FPS 游戏鼠标=视角，移动频繁 → 需要停留判定（2s）防抖动
- 后台播放视频时前台窗口不变 → 由像素运动显著度补充

#### 2.5.2 观察冷却分层（2026-08-19 用户提出）

60s 统一冷却的动机是 **token 成本**，不是模拟真人——真人持续感知、间歇理解。改为**按事件价值分层冷却**（感知层无冷却，只有 LLM 理解有冷却）：

| 事件 | 冷却 | 理由 |
|------|------|------|
| 窗口切换 | 5s | 任务切换 = 注意力转移的高价值信号，真人切换后立即理解新任务 |
| 注视点转移 | 15s | 中价值：看到新区域 → 快速理解「这是什么」 |
| 全屏场景观察 | 60s | 低价值：大图高 token，低频兜底 |

**观察内容分级**（配套 token 控制）：
- 注视区域裁剪（~1/4 屏，低 token）→ 高频观察的主力
- 全屏图（高 token）→ 仅窗口切换后一次

成本估算：注视区域小图一次 ~1-2K token；窗口切换 5s 冷却 → 峰值 ~12 次/分钟 ≈ 24K token/分钟（可接受；实际切换远低于峰值）。
...
2. **鼠标焦点**：鼠标移动到某区域并停留 >2s → 焦点=鼠标位置（日志 `visual: 焦点=鼠标(123,456)`）；鼠标静止 >30s → 焦点=前台窗口标题栏
3. **持续工作不打扰**：用户在文本编辑器持续打字 5 分钟 → 无 LLM 观察触发（打字只是局部帧差，不满足观察时机），仅注视区域停留（日志 `visual: 持续注视`）
4. **显著变化朝向**：屏幕出现弹窗/视频切换 → ≤1s 产生 `fixation_shift` 朝向反应（日志 `visual: 朝向反应 → 显著区域`）
5. **习惯化**：同一静态画面持续 60s → 不再产生观察/扫视事件（日志 `visual: 习惯化`）
6. **分层冷却**：窗口切换后 ≤5s 可再次观察（新切换立即触发）；注视转移 ≥15s；全屏 ≥60s；1 小时 LLM 观察调用 ≤ 峰值上限（分层冷却硬限）
7. **注意力轨迹可查**：`logs/core.log` 每个事件带区域坐标与时间戳，可还原「看了哪里→跳到哪」
8. **回归**：现有 253 测试全过；`vision.py` 迁移后删除（无残留引用）
