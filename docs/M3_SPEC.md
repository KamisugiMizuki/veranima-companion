# M3 专项细化：桌宠端 MVP（docs/M3_SPEC.md）

> 依据：DESIGN.md 3.1（桌宠前端选型风险）、4.6（视觉注意力）、4.7（时空沉浸）、4.8（跨通道共享）
> 进程架构已定（2026-08）：**A 方案——Python 核心进程 + Tauri 纯 UI 壳**。
> 分两阶段：**M3a 纯 Python 逻辑**（时空沉浸/通道互斥/无缝衔接，QQ 端独立验证）→ **M3b 桌宠本体**（Tauri UI + TTS + 在场检测）。

---

## 1. 进程架构（A 方案）

```
┌───────────────────────── Python 核心进程（常驻） ─────────────────────────┐
│  Agent（共享单例：记忆/状态/情绪/依恋度）                                    │
│  ├─ QQ adapter（已存在，ws://127.0.0.1:8099，NapCat）                       │
│  ├─ 时空沉浸引擎（M3a：场景锁 + 后台心跳 + 主动发起仲裁）                     │
│  ├─ 在场检测器（M3b：键盘/鼠标系统事件，L0 零 token）                        │
│  └─ 通道互斥器（M3a：某通道活跃 → 其他通道感知降功耗）                       │
│  └─ 桌宠服务端（M3b：HTTP/WS，供 Tauri 壳连接）                             │
└────────────────────────────────────────────────────────────────────────────┘
                                    │ WS/HTTP（本地，127.0.0.1）
┌────────────────────────────────────────────────────────────────────────────┐
│  Tauri 纯 UI 壳（独立进程）                                                  │
│  - 窗口/桌宠形象/状态气泡（显示「在想你」等）                                 │
│  - 语音播放（接收 TTS 音频流）                                               │
│  - 无业务逻辑，一切转发给 Python 核心                                        │
└────────────────────────────────────────────────────────────────────────────┘
```

**原则**：QQ 与桌宠是**同一 Agent 的两种前端**；桌宠 UI 壳不持有任何状态，断线重连后从核心拉取。

**通信协议（M3b，本地 WS `ws://127.0.0.1:PORT`）**：
| 方向 | 消息 | 说明 |
| --- | --- | --- |
| 核心→壳 | `state` | 状态推送（情绪/依恋度/当前活动） |
| 核心→壳 | `speak {text, tts_tags}` | 语音播放指令（文本 + 语速/停顿标签） |
| 核心→壳 | `bubble {text}` | 状态气泡文案 |
| 壳→核心 | `user_said {text}` | 用户对桌宠说话（语音→文本→Agent） |
| 壳→核心 | `ping` / `pong` | 心跳 |

---

## 2. M3a：纯 Python 逻辑（先做，QQ 端独立验证）

### 2.1 时空沉浸引擎（DESIGN 4.7 细化）

**场景状态锁**：
- 场景识别：用户消息显式声明（「看电影了」「开会」「写稿」）→ 关键词 + LLM 确认双轨
- 场景状态机：`normal → busy（慢回/短回）→ 结束（用户主动回来或 2h 无场景相关消息）`
- 场景期间：主动发起禁止（仲裁器直接拦截）；回复长度上限 ~40 字；回复延迟可加 30~120s（模拟「在忙没看到」）
- 数据：`scene: {type, started_at, last_touch_at}` 存 AgentState（随快照持久化）

**后台心跳**：
- 复用 object_design 的 `OfflineThinkTimer`（静默窗口 + 概率）作为触发源，但**内容改为心跳素材**：
  - 「离线成长」：静默期整理聊天记录（recent_messages 摘要）→ 下次破冰用
  - 破冰模板池：`（刚在整理聊天记录，发现你上次说 X…）` 等，LLM 生成 + 模板兜底
- 心跳频率：静默 ≥30min 后每 2h 最多 1 次（防打扰）；与 4.7 仲裁器联动（闲适性机制）

**主动发起仲裁器**（DESIGN 4.7 已设计，M3a 落地最小版）：
- 输入：五机制请求（M1 已有时间仪式化/社交疲劳/闲适性）+ 场景状态 + 通道互斥状态
- 输出：允许/拦截 + 冷却
- 最小实现：优先级排序 + 频率限制 + 场景拦截 + 互斥拦截（**不做权重公式**，ponytail: 五机制各自独立触发，仲裁只做「拦截+排序」，权重计算等实测需要再加）

### 2.2 通道互斥活动检测（DESIGN 4.8）

- 规则：**任一通道有近期活动（30min 内）→ 其他通道的主动感知/主动发起暂停**
- 实现：`ChannelActivityTracker`（内存，`{channel: last_activity_ts}`）
  - QQ 消息到达 → 记 QQ 活跃
  - 在场检测器（M3b）→ 记桌宠活跃
  - 互斥查询：`active(channel) -> bool`——判断某通道当前是否「独享用户注意力」
- 消费方：仲裁器（QQ 活跃时桌宠不主动说话）、视觉调度器（M4，QQ 活跃时降功耗）

### 2.3 无缝衔接（DESIGN 4.8）

- 触发：在场检测器报告「用户回到电脑前」+ 距上次桌宠交互 >30min
- 行为：Agent 从共享历史取最近 3 条消息 → 生成一句衔接语（prompt：`用户刚离开手机聊天，现在回到电脑前。根据最近对话自然地打个招呼或接续话题，不要说「欢迎回来」这种生硬话`）
- 通过桌宠通道（TTS）输出；若 QQ 最近无对话则不触发（无事发生不硬聊）

---

## 3. M3b：桌宠本体

### 3.1 前端选型（DESIGN 3.1 待定，M3b 开工前定）

| 候选 | 状态 | 备注 |
| --- | --- | --- |
| ~~Tauri v2~~ | ~~MacroPhonic 未调通（窗口/GPU 问题），不能当已验证经验~~ | **已弃选**（2026-08）：Live2D 生态弱 + 透明窗口坑多 |
| **Electron ✓** | koodo-reader 已验证（Electron+React 全家桶） | **已选定**（2026-08）：Live2D 生态最成熟，包大不是问题（可外置更新/按需加载），缺陷对策见 3.5 |
| WPF | Windows 原生 | 弃选：Live2D 生态几乎为零 |

**PoC 范围（M3b 开工第一件事）**：透明置顶小窗 + 点击穿透 + WS 连接，验证通过后进入正式开发。

**评估标准（含 Live2D 前瞻）**：
1. 透明置顶小窗 + 点击穿透（MVP 必需）
2. **Live2D/VTuber 骨骼动画支持度**（中期路线图）：Electron 生态最成熟（pixi-live2d-display），Tauri/WebView2 理论可行但透明+GPU+WebGL 组合坑多（MacroPhonic 教训）——若确认走 Live2D，Electron 加分
3. 与 Python 核心的 WS 通信成本
4. 打包体积与启动速度

**形象渲染抽象**：壳内渲染器只依赖「状态 → 资源」映射（avatar 字段）——静态图/骨骼动画只换渲染器，状态机/协议/交互不动。

### 3.2 TTS 链路

- 复用 MacroPhonic `server.py`（Qwen3-TTS sidecar，已验证）——独立进程，Python 核心 HTTP 调 `/tts`
- 流程：Agent 回复 → TTS 标签解析（`<语速>`/`[pause]`）→ 调 sidecar 合成 → 音频流发给 Tauri 壳播放
- 打断：用户新消息 → 停止当前播放（壳执行，核心发 `stop_speak`）
- 缓存：相同文本 hash 缓存音频（`data/tts_cache/`），避免重复合成

### 3.3 在场检测器

- L0 零 token：Windows 全局键盘/鼠标钩子（`pynput` 或 Win32 API），事件计数 + 时间戳
- 判定：`last_input_ts` 距现在 <5min → 用户在场；持续 >30min 无输入且屏幕锁定 → 离开
- 只发事件：`presence: on/off`，不传具体内容（隐私）

### 3.4 Electron 显著缺陷与对策（2026-08 选型后专项）

桌宠是 **7×24 常驻**应用，Electron 的缺陷必须正面处理。逐条对策：

**缺陷 1：渲染进程内存泄漏（Chromium 多进程模型）**
- 症状：长时间运行后渲染进程 RSS 持续增长（常见泄漏源：未清理的事件监听器、闭包引用、pixi.js 纹理/精灵未销毁、`<audio>` 元素残留）
- 对策：
  - **渲染进程心跳监控 + 自愈重启**：主进程每 5min 采样 `webContents.getProcessMemoryInfo()`，渲染进程 RSS 超阈值（如 400MB）或持续增长（3 次采样均升）→ `webContents.reload()` 或重建窗口。壳无状态，重启无损（状态在 Python 核心）
  - **事件监听纪律**：React 组件卸载必清监听（`useEffect` cleanup / `AbortController`）；全局监听器（WS、全局快捷键）挂 main 进程不挂 renderer
  - **音频资源释放**：`<audio>` 播放完置 `src=""` 并移除节点；TTS 音频是高频对象，是泄漏重灾区
  - **pixi.js 资源纪律**（Live2D 后启用）：纹理/模型卸载时 `destroy()`，不用 `removeChild` 了事
  - **V8 堆上限**：`app.commandLine.appendSwitch('js-flags', '--max-old-space-size=512')` 防堆失控

**缺陷 2：渲染进程崩溃/白屏**
- 症状：GPU 崩溃、OOM 后窗口白屏无响应
- 对策：监听 `render-process-gone` 事件 → 自动重建窗口（含当前状态恢复）；配 `process.crashReporter` 落盘日志便于排查

**缺陷 3：GPU 进程问题（透明窗口 + WebGL）**
- 症状：透明窗口花屏/闪烁、GPU 进程崩溃（MacroPhonic 同款坑）
- 对策：PoC 阶段就验证「透明 + 置顶 + WebGL(为 Live2D 预留)」组合；出问题先试 `app.disableHardwareAcceleration()` 或 `--use-gl=angle --use-angle=swiftshader`（软件渲染，桌宠画面简单，性能损失可接受）

**缺陷 4：后台节流（timer 被压）**
- 症状：窗口隐藏/失焦时 Chromium 节流 `setTimeout`/`requestAnimationFrame`，呼吸动画/气泡消失卡顿
- 对策：**壳内动画用 main 进程定时器驱动**（`webContents.send` 广播），不依赖 renderer 的 rAF；或 `powerSaveBlocker.start('prevent-app-suspension')`（常驻桌宠合理使用）

**缺陷 5：包体大 + 启动慢**
- 症状：~100MB 安装包、首启几百 ms
- 对策：**MVP 阶段接受**（本地开发跑 dev 模式无感）；后续优化路径：electron-builder 压缩 + 按需加载 Live2D 资源（模型文件不进主包，运行时从 `assets/` 读）+ 可选 ASAR 外置。**不提前优化**（YAGNI：等真到了分发阶段再处理）

**缺陷 6：单实例冲突**
- 症状：用户重复启动出现两个桌宠
- 对策：`app.requestSingleInstanceLock()`，第二次启动 → 聚焦已有实例并退出

**监控与自愈总纲**：
- 壳内建 `health.js`（main 进程）：5min 周期采样渲染进程内存 + WS 连接状态 → 异常自动恢复（重启渲染进程/重连核心）
- 所有自愈动作打日志（`logs/pet-shell.log`），配合核心侧日志可完整复盘

---

### 3.5 桌宠前台 UI 细节（MVP 必需集）

**窗口形态**：
- 尺寸 ~200×240px（形象区 + 气泡区），置顶 + 无边框 + **点击穿透**（除形象本体外区域鼠标穿透，不挡用户操作）
- 位置：默认右下角（任务栏上方），可拖拽（按住形象本体拖动）；记住位置（本地 config）
- 最小化=隐藏到托盘（不退出核心）；托盘图标点击恢复

**形象（MVP = 静态图 + 状态切换，不做骨骼动画）**：
- 状态机：`idle`（默认，轻微呼吸动画可后置）/ `speaking`（TTS 播放中）/ `thinking`（核心处理中）/ `sleeping`（用户离开）
- 形象来源：角色卡配置 `avatar` 字段（图片路径）；MVP 先提供 2-3 张占位状态图，动画后续换

**状态气泡**：
- 触发：`speak` 消息的文本同时进气泡（听+看）；`thinking` 显示「……」（打字感）
- 气泡样式：圆角白底/黑字，跟随形象；**不常驻**，无消息 3s 后消失（防挡屏）

**用户交互（MVP 最小集）**：
- 左键单击形象：触发「戳一下」→ 核心生成一句互动（复用 heartbeat 的破冰逻辑，短版）
- 右键菜单：`说话（开关）` / `去安静会儿`（进 away 场景）/ `退出桌宠`（只退壳，核心继续跑 QQ）
- 拖拽：按住移动位置
- **MVP 不做**：语音输入（麦克风转写）、拖文件投喂、形象动画编辑器

**生命周期**：
- 启动：壳启动 → 连核心 WS（失败重试，指数退避 1s→2s→4s…上限 30s）→ 拉取 state → 显示
- 退出：壳退出不杀核心（核心是常驻服务）；核心退出时壳显示「失联」气泡 + 托盘重连
- 崩溃恢复：壳重启后自动重连，状态从核心拉取（壳无状态，断线无损失）

**通信细节**：
- 协议沿用 3 节表；`speak` 带 `tts_tags`（语速/停顿），壳解析后按标签播放
- 心跳：壳每 10s ping，核心 3 次无响应判定断线 → 显示失联

### 3.6 单窗口架构（airi 式，2026-08 定案）

**总则**：全系统只有一个可见窗口（Electron 壳）。Python 核心由壳 spawn 管理；桌宠态 ↔ 控制台态同窗口切换。

**进程模型**：
- 壳启动 → `child_process.spawn(python, [pet_server.py / qq.py], {stdio: ['ignore','pipe','pipe']})` 拉起核心
- 壳监控核心：`exit` 事件 → 指数退避重启（同 WS 重连策略）；核心崩溃不拖垮壳
- 壳退出策略：托盘「退出」= 壳+核心一起退；关窗（X）= 隐藏到托盘（核心继续）
- 核心的 WS 端口改内部固定（8765），壳重连逻辑不变

**窗口形态切换**（同一 BrowserWindow，不新建窗口）：
| 形态 | 尺寸 | 内容 | 触发 |
| --- | --- | --- | --- |
| 桌宠态 | 220×260 透明置顶 | 形象 + 气泡（现状） | 默认 / 控制台收起 |
| 控制台态 | 800×600 不透明 | 顶栏（返回桌宠）+ 设置页 + 日志页 | 双击形象 / 托盘菜单「打开控制台」 |

- 切换实现：`win.setBounds` + 透明开关（`setBackgroundColor`）+ renderer 布局切换
- 控制台态时点击穿透关闭（可交互）；返回桌宠态恢复穿透

**控制台内容（MVP）**：
- **设置页**：config.yaml 只读展示 + 关键字段（LLM base_url/model、QQ 白名单）编辑 → 保存后提示「重启核心生效」（MVP 不做热重载）
- **日志页**：核心 stdout/stderr（壳捕获转发 `log-line`）+ 壳自身日志（console 重定向）+ 清空按钮；环形缓冲（内存 500 行，防膨胀）
- **进程状态**：核心运行中/已崩溃（自动重启中）/已停止；「重启核心」按钮

**实现拆分**：
- 第一步（最小）：壳 spawn 核心 + 日志转发 → 控制台日志页（验证单窗口闭环）
- 第二步：形态切换 + 设置页（config 编辑/重启生效）

---

## 4. M3 验收清单

| 子块 | 验收 |
| --- | --- |
| 2.1 场景锁 | 「看电影」→ 回复变短变慢；主动发起被拦截；用户回来说「看完了」→ 恢复 |
| 2.1 心跳 | 静默 30min 后 2h 窗口内最多 1 次破冰，内容带「离线整理」痕迹 |
| 2.2 互斥 | QQ 活跃 30min 内桌宠不主动；桌宠互动后 QQ 主动发起正常 |
| 2.3 衔接 | 回到电脑前触发衔接语，内容与最近 QQ 对话相关 |
| 3.2 TTS | 桌宠语音正常合成播放；新消息打断旧播放 |
| 3.3 在场 | 键盘活动 5min 内判定在场；30min 无输入判定离开 |
| 3.5 窗口 | 置顶无边框 + 点击穿透；拖拽移动并记住位置；最小化进托盘 |
| 3.5 形象 | idle/speaking/thinking/sleeping 四态切换正确 |
| 3.5 气泡 | speak 文本进气泡，3s 后消失；thinking 显示「……」 |
| 3.5 交互 | 左键「戳一下」触发互动；右键菜单三项可用；壳退出不杀核心 |
| 3.5 重连 | 核心重启后壳自动重连恢复（指数退避） |
| 3.4 内存 | 渲染进程 RSS 超阈值自动重启；7×24 运行无白屏 |
| 3.6 单窗口 | 启动后屏幕只有一个窗口（壳）；核心由壳 spawn，崩溃自动重启 |
| 3.6 控制台 | 展开可见核心/壳日志；设置页保存后重启核心生效；返回桌宠态恢复穿透 |

---

## 5. 与 DESIGN.md 的关系

本文件是 DESIGN.md 4.6/4.7/4.8 的 M3 专项细化；M3 完成后核心结论（进程架构/协议/选型结果）回写 DESIGN.md 3.1。
