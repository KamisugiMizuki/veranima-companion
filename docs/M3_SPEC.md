...
### 3.6 窗口架构（airi 式多窗口，2026-08 定案）

**总则**：main 进程统一管理所有窗口（airi `window-manager` 模式）。主窗口（桌宠）+ 设置窗口 + 日志窗口，均独立 BrowserWindow，可复用窗口模式创建。

**主窗口（桌宠）**：
- 450×600（airi 主窗口尺寸），透明 + 置顶 + `type: 'panel'`（airi 同款：置顶层级更高，不抢焦点）
- **关窗 = 隐藏**（`close` 事件 preventDefault + hide；airi `allowClose` 模式）——托盘常驻，只有托盘「退出」真关
- 位置/尺寸持久化：`move`/`resize` → 写 config.json（airi `windows` 数组同款）；重启恢复
- 双击形象 / 托盘菜单 → 打开设置窗口

**设置窗口**（airi settings 窗口同款）：
- 独立 BrowserWindow 600×800，非透明、非置顶
- `createReusableWindow` 模式：首次打开创建，关闭后复用（不销毁，`show`/`hide` 切换）
- 内容：config.yaml 只读展示 + 关键字段编辑（LLM base_url/model、QQ 白名单）→ 保存提示「重启核心生效」（MVP 不做热重载）

**日志窗口**：
- 独立 BrowserWindow 500×600，复用模式
- 内容：核心 stdout/stderr（壳捕获转发 `log-line`）+ 壳自身日志 + 清空按钮；环形缓冲（内存 500 行）
- **文件日志**：全部日志同时落盘 `userData/veranima-{ts}.log`（airi file-logger 同款：时间戳文件、无需轮转、可追溯会话）

**进程模型**：
- 壳启动 → `child_process.spawn(python, [pet_server.py / qq.py], {stdio: ['ignore','pipe','pipe']})` 拉起核心
- 壳监控核心：`exit` 事件 → 指数退避重启（同 WS 重连策略）；核心崩溃不拖垮壳
- 壳退出策略：托盘「退出」= 壳+核心一起退；关窗（X）= 隐藏主窗口（核心继续）
- 核心的 WS 端口固定（8765），壳重连逻辑不变

**实现拆分**：
- 第一步（最小）：壳 spawn 核心 + 日志转发 + 日志窗口（验证多窗口闭环）
- 第二步：主窗口关窗隐藏 + 位置持久化 + 设置窗口
...
| 3.6 主窗口 | 主窗口透明置顶（type=panel）；关窗隐藏不退出；位置持久化恢复 |
| 3.6 设置窗 | 独立设置窗口可打开/复用；保存配置后重启核心生效 |
| 3.6 日志窗 | 日志窗口实时显示核心/壳日志；文件日志落盘 userData |
| 3.6 进程 | 核心由壳 spawn，崩溃自动重启 |
