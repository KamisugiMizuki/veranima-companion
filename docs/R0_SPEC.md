...
8. 失败：从原文提取可读文本；完全失败返回 `degraded`。LLM/连接故障兜底和结构化/分析协议残片可以作为本轮降级返回，但不得写入 assistant 历史、记忆或风格学习；历史中已有的内部输出也必须在读取时过滤。
9. 主动消息若使用历史素材，必须直接点明可靠主题；内部关系张力事件不是聊天素材。紧邻主动消息的“啥事/什么事”等追问只复用该主动消息上下文，不重新猜测候选，不输出分析草稿。
10. QQ 三餐提醒由现有主动 Gate 驱动：早餐约 08:00、午餐约 12:00、晚餐约 17:00，各自前后 10 分钟内稳定随机一次；受静默时段、通道上限、间隔和发送后去重约束。
