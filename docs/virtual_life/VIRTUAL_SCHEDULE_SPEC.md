...
### 5.7.1 生活的持续后果（设计补全，尚待实现验收）

生活主题不能只换活动名称。复用现有主题、牵挂、活动事件与日程约束承载以下连续性，不另建身体模拟器或多智能体社会：

- 角色自己的目标有下一步、阻碍、进展与退出条件；投入活动后可以失败、暂停或放弃，不因时间到点必然成功。未向用户分享也可在合法模拟事件中推进，日后追问保持一致。
- 身体限制按角色已有设定与有来源事件表达：疲劳、欠睡、临时不适应影响后续安排并有恢复条件；不只改语气，也不每天随机造病。没有依据不添加疾病或永久属性。
- 角色卡已有家人、同事或朋友时，维持稳定身份与关系；第三人的约定与职责可以约束可选日程，取消应有后果。没有设定则不强造社交圈，也不替现实用户联系他人。
- 计划、实际模拟事件、角色获知和说出口分开；未知活动不补成完成，反思只能改变解释与未来意向。

验收：连续多日追问同一目标可见一致进展或停滞；临时不适恢复前不无缘无故消失；既定职责不因一条普通聊天自动取消；用户不在场时的合法事件能被后续对话消费。
