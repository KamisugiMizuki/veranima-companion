...
### 6.4 入库前蒸馏（粒度闸，2026-09-08）

真库审计（`exports/mumu_20260908.db`）发现「条目太具体」：记忆库页 35 条可见条目里 25 条是用户原话直存——「不是，没反应了？」被当用户事实、内部 slug、粘贴的聊天记录整块。存的是"用户说过什么"，而不是"用户是什么样的人"。写入侧加一道粒度闸：

- 命中记忆点（规则提取 / 判断点 `memory_kind`）时，先把原话蒸馏成一句第三人称事实句（主语「用户」，≤40 字，保留时间/对象/原因，不脑补不评价）再入库；当天即可召回。
- 模型判定「无记忆价值」（寒暄、情绪宣泄、对助手的指令、纯问句、表情）→ 不写入，也不进待审收件箱。
- 蒸馏未裁决（离线/报错）→ 回退旧行为存原话，宁可粗不可丢。
- 不再二次蒸馏：用户明确说「记住」的即时类、`manual` 来源、夜间整理产物、判断点已产出的第三人称短句、张力账本。
- 承诺条目沿用 `承诺：` 前缀，只蒸馏正文，`meta.status` 等字段原样保留。

存量逐条处理（`maybe_distill_backfill`）：每 tick 最多 2 条，改写走版本链（原话留在链上可审计）、判定无价值则整链删除、未裁决留待下次；处理过的条目打 `meta.distilled_at`，重跑不再调模型。Android bridge 与 QQ 适配器各挂一次，桌面端由写入侧闸门负责。
...
### 10.5 记得、查得到与在场不是同一件事（设计修订）

- 共享用户事实继续共享；事件的参与角色、转述来源和承诺持有者分别保留。未在场角色可按共享策略知道一件事，不能说成「你当时对我说」或冒领共同经历。
- 自动召回只供给相关片段；原始聊天保留不等于角色每轮都精确记得。用户明确要求翻记录时，才按已有消息检索链提供可核原话，并区别自然回忆与查到的记录。
- 纠正后消费当前有效解释，保留过去语境；不以旧摘要覆盖用户当轮澄清。不新增随机失忆或内容级出口硬删机制。
- 验收：跨角色追问不冒领在场、普通回忆与主动翻记录可区分、纠正后重启不复活旧解释。
