...
> 技术约束：继续使用 SQLite + FTS5 + sqlite-vec + 本地 bge-m3；不引入 Mem0、Memanto、MemPalace、图数据库或远程记忆服务作为运行时依赖。2026-08-26 选型评估结论：维持 SQLite 栈不变，吸收外部记忆库机制（热度衰减/双时间线/夜间整理/审核收件箱）以增量方式落地，评估与阶段见 `docs/MEMORY_BACKEND_EVAL.md`。
...
1. semantic cosine：sqlite-vec + bge-m3。
