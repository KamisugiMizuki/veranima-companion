...
- **回复落点**：turn 在跑时新消息归下一轮，回复自然晚于输入——「回复对的是几条
  之前的消息」必须保留本轮输入批次的归属。历史仅提供语境，不能代替回复对应关系；迟到回复必要时用已有引用能力指向原消息，不让用户猜在答哪一句。
...
## 5.1 连发纠正与恢复（设计修订，尚待实现验收）

- 保留生成中不打断的裁决；发送前检查本轮对象是否已被明确取消或更正。用户新发的「不对，改成……」归下一轮，但不得把已知失效的旧建议当成当前结论发布。无法判断时保留原批次归属，下一轮解释更正，不猜意图。
- 入队、持久化、处理、可见交付分别记清。重启以未处理消息身份恢复，不能只凭最后一条是 user 或时间窗推测整批已答。超窗可停止自动补答，但不得静默标为已处理。
- 合并保留各条消息及图片的对应关系；批次末条标识不能代表所有证据。重试不重复入库或重复发布。
- 输入中指示只对应真实等待/生成状态，失败后退出；技术错误由应用给重试入口，不让角色以「忙」「不想聊」掩盖。

验收：连发纠正、两张图各带不同问题、回复期间切角色、入队后杀进程、生成后交付前中断，均检查消息归属与一次交付。
