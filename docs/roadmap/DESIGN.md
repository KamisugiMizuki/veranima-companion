...
> 设计审计修订：本轮补正体验契约；新增与变更行为尚未据此改码或完成实机验收，历史实现标记仅对应修订前范围。见 [审计报告](audits/REALISM_DESIGN_AUDIT.md)。
...
拟真表达由五维控制面协同：认知过程、情绪耦合、表层人格演化、关系动态和有因差异。认知过程只输出短结构 `ResponsePlan`，不生成或暴露私密思维链；情绪使用连续 PAD（愉悦度/唤醒度/支配度）并具有 cause、惯性和衰减；差异性由状态、关系、注意力与自然联想共同形成；允许角色说不清原因，但不通过固定概率注入人格反常。系统留痕与角色自知分开。
...
分两组：**拟真组**回答“她像不像一个人”，**工程组**回答“她稳不稳定”。工程组的绿灯不能冒充拟真组的绿灯。“我不在的时候她也在”是独立生活维度的判据；同时检查稳定身份、有限认知、因果连续、关系边界和对话自然度。离场留痕不能替代当面对话质量，次数指标用于采样，不得驱动角色刻意制造回忆、情绪或主动话题。
...
- 角色可以主动不开口——低落、忙碌或正在处理自己的事时保持沉默，沉默不产生愧疚或补偿性解释；回复时处于自己的时间线上（可能正忙、刚醒、在外面），不是永远待命。她自己的情绪不构成拒绝用户倾诉或降低必要回应质量的理由。模板明确的睡眠与不可交流活动仍按日程契约延后处理，恢复后接续；这不承诺全天即时在线。技术失败由应用说明，不伪装成角色生气或睡着。
