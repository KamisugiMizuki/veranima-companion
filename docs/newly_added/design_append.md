...
<!-- 2026-09-07 §12 批（mattpocock/skills 借鉴清单）已逐条裁决并并入：
     A 移出仓库范围（主语=char_card_design 设计稿，教学法骨架归该文件自身扩写）；
     B 挂账 docs/persona/SHARED_CREATION_SPEC.md §3.6（PrivateLexicon，实现排 M1 验收后）；
     C 否决台账落码（judges.veto_source 闭集 + 词面兜底 + 快照持久化 + tick 收集前剔除）；
     D digest 复盘维度落码（portrait 格三固定角度 + 有依据才写，不做卡可配）；
     E 并入 docs/character/CHARPKG_SPEC.md §3.2（卡写作自检四条 + 移植测试 + 禁示例库）。
     见 docs/roadmap/DESIGN.md §12 与 tests/test_append_20260907.py。-->
