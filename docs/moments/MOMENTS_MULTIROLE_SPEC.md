...
入口=角色私产页（§6，聊天顶栏齿轮进入），本节只列设置项本体。按「设计稿全量、分期实装」列（勾=Phase）：
...
**分家原则（2026-09-01 用户 UI 裁决）**：设置页只放**角色无关**的全局项；
一切「关于某个具体角色」的东西从设置页撤出，收进该角色聊天界面的
顶栏右端入口（角色私产页）。入口归属如下表：

| 内容 | 现居 | 新归属 |
|---|---|---|
| LLM / Embedding / 搜索 API 配置 | 设置 | **留设置**（全局） |
| 活跃角色切换 | 设置 | **留设置**（全局：选默认对外应答的卡） |
| 共享记忆导入/导出 | 设置 | **留设置**（全局） |
| 共同记忆库（memory_detail） | 设置 | **留设置**（全局：单脑共享） |
| 用户睡眠报告（设备侧入睡/苏醒） | 设置 | **留设置**（全局：是"用户"的作息不是角色的） |
| 电池优化白名单 / 使用情况访问 | 设置 | **留设置**（系统权限，全局） |
| 羁绊图谱（relationship_detail） | 设置 | **移入角色私产页**（关系是角色×用户的私产） |
| 角色当前作息卡（原混在睡眠报告内） | 设置·睡眠页 | **拆出移入角色私产页**（"用户睡眠"留设置，"角色作息"归角色） |
| 导出/删除当前角色及其私产 | 无 | **角色私产页**（新增；含重置记忆/亲密、称呼锁定） |
| 角色设置开关组（§5 主动/动态/互动…） | 无 | **角色私产页** |
| 导入角色包（.char） | 设置 | **移入角色列表页顶栏右端** |

```
底栏（三 tab，icons：chat/feed/settings — material icons 现有依赖内选）
├ 聊天 = 角色列表页
│   顶栏：应用名 Veranima 居中｜右端=＋加号图标（新角色入口：点开=导入
│   .char 角色包；语义=往通讯录里加人，不是设置）
│   条目：圆角正方形头像（用户供图，无图回退首字母几何块；尺寸同微信列表
│         ~52dp，圆角 ~12dp，1dp 黑边）+ 角色名 + 最后一条预览 + 时间
│         + 未读红点（role_reads 差值计数）；长按=仅此条会话标记/删除
│   ↓ 点入 = 现有聊天页复用（立绘背景/气泡/输入栏全保留）
│       顶栏：返回｜角色名居中｜右端=齿轮图标 → 该角色私产页
│       （两级顶栏图标不同：列表页=＋加号[全局导入]，会话页=齿轮[角色私产]）
├ 好友动态 = 信息流页
│   分组按日；条目=圆角方头像+角色名+时间+正文（≤100 字无折截断）+
│   底部 [♡ n][评论] 两枚描边胶囊；点开=评论列表 + 输入框
│   角色发布节奏：空白日自然留白，不做"暂无动态"占位轰炸
└ 设置 = 纯一列 GalaxyNavRow（裁决 UI-4：大块内容一律收纳进二级页，
    主页零表单、零内联编辑框）
    ├ API 配置          → 二级页 api_detail（LLM/Embedding/搜索三组表单整体搬入）
    ├ 当前活跃角色      → 二级页 active_role（单选列表，对外应答默认卡）
    ├ 共享记忆备份      → 二级页 backup_detail（导出/导入 zip）
    ├ 共同记忆库        → memory_detail（现有，留）
    ├ 用户睡眠报告      → sleep_detail（现有，瘦身为纯用户侧——角色作息卡迁出）
    ├ 电池优化白名单    → 行内跳转系统设置（一行即达，无需二级页）
    └ 使用情况访问      → 行内跳转系统设置（同上）
    每行副标带状态摘要（如 API 行="DeepSeek · deepseek-v4-flash"、活跃角色行=
    角色名），信息不点进去也能扫一眼全有

角色私产页（聊天顶栏齿轮进入，NavHost 路由 `role_space/{role_id}`）：
┌ 圆角方头像 + 角色名 + 「关系类型·城市」副标
├ 羁绊图谱        → relationship_detail（复用，读该角色 roster 条目）
├ 角色当前作息    → 从睡眠页迁来的作息卡（含 max_offset 上限显示）
├ 主动行为/好友动态/互动/称呼与表达 开关组（§5）
├ 导出此角色（.char 私产包）· 重置记忆 · 重置亲密度（二次确认）
└ 说明行：「以上仅影响 {角色名}，不波及其他角色」
```

设置内嵌 NavHost 路由：`role_space/{role_id}`（角色私产页，从聊天齿轮进入，
非设置页进入）；memory_detail/sleep_detail 留设置；relationship_detail 现挂
`role_space` 之下作二级页。头像图由用户提供，落 `characters/<id>/portrait.jpg`，
缺失回退首字母几何块（现有羁绊页首字圆形同款逻辑，改方形）。
...
| UI-1 | 页面分家 | 设置页=全局项（三 API/活跃角色/共享记忆导入导出/共同记忆库/用户睡眠报告/电池白名单/使用情况）；角色私产（羁绊图谱/角色当前作息/角色设置开关/导出删除角色）全部移入**聊天顶栏右端齿轮→角色私产页** |
| UI-2 | 导入角色入口 | 角色列表页顶栏右端=**＋加号**（移出设置页）；进入单角色会话后顶栏右端=**齿轮**→该角色私产页——两级图标语义分离 |
| UI-3 | 列表头像 | 圆角正方形，图由用户提供（characters/<id>/portrait.jpg），缺图回退首字母几何块 |
| UI-4 | 设置页收纳 | 主页=纯一列 GalaxyNavRow 零表单；API 配置/活跃角色/共享记忆各包成二级页（对齐羁绊/记忆库/睡眠的既有收纳风）；系统权限两项保持行内直跳；行副标带当前值摘要 |
