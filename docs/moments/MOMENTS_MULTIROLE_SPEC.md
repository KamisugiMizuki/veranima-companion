...
质量线（与现落码一致）：只有角色化骨架或通过可见文本检查的自然摘要可以降级发布；无合格文字则不发布（记失败账，同日不重试，禁止把活动键、数值、内部判词或素材原文当成品上屏）。
每次生成带 `source_ref` 供内部溯源；可见通道零协议泄漏。
...
主动消息不补（过期问候不如不说——合并窗口哲学同源）。动态须区分事情发生时间与实际发布时间；无有效摘要就留白，不将计划或离线未知活动改写为完成。恢复后也不把补生成伪装成早已发布。
