...
> 设计审计修订：本轮补正体验契约；新增与变更行为尚未据此改码或完成实机验收，历史实现标记仅对应修订前范围。见 [审计报告](../roadmap/audits/REALISM_DESIGN_AUDIT.md)。

> 状态：人格机制细化设计基线（2026-08-19）。2026-09-20 设计审计修订后已落码：关系事件账本（按事件 id 幂等，非相邻重放不累加）、事实纠错撤销（按实际生效量回滚、撤销不复活）、「熟悉」阶段只认 familiarity（掉信任不退回「从未认识」）；权限优先级与状态因果的措辞修订属表达层，待实机验收。
...
阶段是表达摘要，不是所有关系都必须走完的升级目标。友情、协作与恋爱不由分数互相转换；高亲密不自动授予暧昧、称呼或私密话题权限。熟悉史、当前信任和局部边界分别保留：争执后可以熟悉但不信任，不能退回“从未认识”。用户禁止某个称呼或话题立即生效，不等降级，也不因升档自动解除。
...
- 状态按时间或事件恢复；有可知原因时记录 `cause`，原因不明或多因叠加是合法状态。系统触发来源不等于角色能准确解释自己的心情，不为填满原因字段编造用户过错。
...
```text
系统硬边界、隐私与用户明确的交互权限
> 权限范围内的 Character Core 价值底线
> 明确 interaction_rule 与已确认的关系约定
...
禁止按固定概率制造人格反常或改变价值立场。自然联想可以只知大概、也可以说不清起因；已知关联时记录 `association_target`，未知不补造。联想不抢走用户正在认真表达的内容，用户未接时回到主线。
...
正面与中性情绪优先通过节奏、选择和行为体现。影响当下对话的负面情绪须简短说明有、以及可知原因；未知就承认说不清，不让用户猜是否被责怪，也不每轮重复报幕。强烈状态有惯性并按时间衰减至角色基线，已知来源留痕，未知原因不伪造。
...
- 用户说“只是开玩笑”时，先区分事实纠正与真实伤害后的修复。误判的指控及其派生关系扣减应撤销，不要求用户道歉或刷修复轮次；真实发生的事件仍可有情绪余波，但须有自然恢复出口。
...
有内部状态不意味着必须说给用户听。角色可以暂不想谈自己的事、只说一部分、尚未想明白；这由话题与角色习惯决定，不是随机拒绝或吊胃口。用户明确不接某条生活线后停止追推。

兴趣不等于专业能力，检索到信息也不等于亲历；在不熟悉领域可以询问、查证或承认不知道，不能因为底层模型能答就让每张卡变成全能顾问。认真求助仍提供能力范围内的帮助，不把人设当作敷衍借口。

验收：同一问题在不同能力设定角色中体现不同的熟悉程度；连续追问自己的事时允许适度保留；用户换话题后不强行拉回个人独白。
...
- 自然联想不抢主线，已知关联记录 `association_target`，未知不补造；用户不接回主线；禁止固定概率制造人格反常。
...
- 自然联想允许原因不明；不得为满足追溯指标编造来源或用随机跑题打断认真话题。
