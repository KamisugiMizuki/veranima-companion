...
> 状态：实现基线 v1.0；T0–T4 已按本规范落地。2026-09-20 设计审计修订后的恢复语义已落码——
> 表达档随数值自然退出（不再要求修复轮次）、用户澄清触发误判撤销（不消耗正向额度、
> 跨重启不复活）、事件按 id 幂等（非相邻重放不累加）；其余条款待实机验收。
...
- 用户纠正“你误会了”时，核对后将不成立事件标为 `superseded`，撤销其尚存派生影响；这是纠错，不消耗正向每日额度，也不要求用户赔偿性互动。有效事件修复后标为 `resolved`。审计保留仍服从用户的隐私删除权。
...
### 5.1.1 交付事实与期待归属（设计修订）

入库、发送成功、设备通知展示、用户读过、用户愿意回答是不同事实。发送成功只允许记录「没等到回应」，不得推出已读或恶意。应用不可达、通知失败、用户明确忙碌或拒绝话题时，取消或暂停相应期待；不可把技术失败记进关系账。

期待按角色与具体话题闭合：用户任意回话可以结束催问，不必证明回答了问题；事项是否完成仍按事实另行判断。沉默允许带来角色自己的轻微失落，但不能把自动发出的问句变成用户必须履行的约定。取消期待后不得换同义问句重新施压。

验收：通知失败不生关系扣减；用户说「不想谈」立即停止该线追问；角色甲收到回复不等于角色乙的约定兑现；普通分享无回复不生成欠账。
...
- 进入 `repair`：TV >= 61；数值降到 ≤48（含）可经有效修复或自然冷却退出；
- 进入 `high`：TV >= 81；数值降到 ≤65（含）可经有效修复或自然冷却退出。修复轮次可以提供缓和证据，不是用户必须完成的配额。
- 无新负向证据时，表达档不得在数值归零后停留于 `repair/high`。仍需维护的边界单独保留，不靠持续冷淡维持。
...
- 有效互动提供缓和证据，不能仅按轮数把真实冲突视为解决；
- 时间冷却允许情绪自然缓和，事实纠错直接撤销错误影响；两者都不要求用户完成互动配额；
- 未解决的实际分歧可以继续保留为议题或边界，不把它等同于永久负面表达；
...
| 模式 | 表达 |
|---|---|
| `restrained` | 变短、变克制；影响对话时简短交代负面与可知原因，不直接控诉 |
| `direct` | 可以明确陈述“这件事我在意”，但不攻击人格 |
| `hurt` | 允许表达委屈和不确定，优先请求澄清 |
| `neutral` | 主要改变节奏和主动性；不隐瞒影响对话的负面，不把未回复断言为怠慢 |
...
- 有效互动可促成软化，但不以固定轮数判定修复；无新负向证据、数值归零时高张力表达必须退出；
- 事实误判撤销不受正向日额度限制；重启后不能恢复已撤销影响；
