...
### 3.6 PrivateLexicon（共享语言·挂账概念，暂不实现）

> 2026-09-07 从外部借鉴评估（design_append §12-B）并入。概念留档，实现排期挂在
> M1 画像肉眼验收之后——这是第一个**运行时可写**的 user×role 词汇实体，不是小改动。

用户与角色在对话中自发产生的缩略语、梗、专属叫法（「上次那个事」「屋顶时间」），
是关系深度的最自然证据。现有机制里昵称池（卡上 `nickname_pools`，运行时只读）
是这套东西的「称呼」特例；泛化目标 = **每角色私有词表**：

- **入库门槛（硬）**：词必须是双向确认过的——用户造词、角色复述一次、用户默认
  或接续，才算登记。单向出现（角色自己脑补的「我们的梗」）= 上次「翻聊天记录」
  幻觉的同款病灶，必须拦。
- **阶段解锁**：初识期不会使用只有亲密期才懂的梗；词表条目带登记时的关系阶段，
  引用时校验当前阶段 ≥ 登记阶段。
- **召回联动**：主动消息/追问命中私有词时，语义上等于「记得我们的事」，走既有
  episodic 联想链路带出。
- **实现形态（预定）**：memory 的一个新 kind + 注入时的一个补充块；不建独立
  平行人格库（与 §3 总原则一致）。衰变走既有 strength 机制，不造第二套遗忘。
- **明确不做**：全局共享词表（换角色必须隔离，红线「换反差角色不泄漏旧角色
  词汇和锚点」）、用户手动导入词条（违背「自发产生」的命题）、词表编辑 UI
  （M1 验收前不开新设置面）。
